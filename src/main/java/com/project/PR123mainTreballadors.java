...
public class PR123mainTreballadors {
    ...
    public void mostrarTreballadors() throws IOFitxerExcepcio {
        ...
        UtilsCSV.llistar(treballadors);  // Utilitza UtilsCSV per mostrar les dades
    }
    ...
    public void modificarTreballadorInteractiu() throws IOFitxerExcepcio {
        ...
        // Mostrar modificacions i demanar si es volen guardar
        System.out.println("\nModificació completada. Vols guardar els canvis al fitxer? (sí/no)");
        String resposta = scanner.nextLine();
        if (resposta.equalsIgnoreCase("sí")) {
            List<String> treballadorsModificats = llegirFitxerCSV();
            escriureFitxerCSV(treballadorsModificats);  // Actualitzar el fitxer
            System.out.println("Els canvis s'han guardat correctament.");
        } else {
            System.out.println("Els canvis no s'han guardat.");
        }
    }

    // Mètode que modifica treballador llegint i escrivint sobre disc
    public void modificarTreballador(String id, String columna, String nouValor) throws IOFitxerExcepcio {
        List<String> treballadors = llegirFitxerCSV();

        // Obtenir el número de línia del treballador amb l'ID
        int liniaTreballador = UtilsCSV.obtenirNumLinia(treballadors, "Id", id);
        if (liniaTreballador == -1) {
            System.out.println("No s'ha trobat cap treballador amb l'ID proporcionat.");
            return;
        }

        // Actualitzar la línia amb el nou valor
        UtilsCSV.actualitzarLinia(treballadors, liniaTreballador, columna, nouValor);
    }
